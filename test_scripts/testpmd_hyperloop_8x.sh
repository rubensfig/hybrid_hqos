...
function run_testpmd_sriov() {
    local i=$1
    local pci=$2
    local cores="$((BASE_CORE + i * 2))-$((BASE_CORE + i * 2 + 8))"
    BASE_CORE=50
    local prefix="vf$i"
    local fwd_mode=$3
    ...
    $DPDK/build/app/dpdk-testpmd \
        ...
        $eth_peer_flag --rxd=1024 --txd=1024 --txq=8 --rxq=8 --nb-cores=8 --auto-start \
...
function run_testpmd_memif() {
    ...
    $DPDK/build/app/dpdk-testpmd \
        ...
        $out_eth_peer --rxd=1024 --txd=1024 --txq=8 --rxq=8 --nb-cores=8 --auto-start \
        > /tmp/testpmd_${prefix}.log 2>&1 & disown
