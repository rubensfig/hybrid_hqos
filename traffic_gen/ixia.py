...
EXPSET = "LS"
EXP = "profile1"
SAVE_DIR = r"C:\Users\Administrator\Documents\rubens\saturn\hybridhqos"
...
json_file_path = r"C:\Users\Administrator\Documents\rubens\waldorf\software_hqos\experimental_description\hybridqos.json"
...
def topology_setup(ixnetwork, traffic_items):
    ...
    for i in range(0, len(traffic_items)):
        ...
        ixnetwork.info("Add US Device Group=" + "Upstream-" + traffic_item_name)
        ...
        ixnetwork.info("Add DS Device Group=" + "Downstream-" + traffic_item_name)
...
def get_background_loops():
    ...
    if "levels" in bg_config:
        ...
        prio_configs = {key: value for key, value in bg_config.items()}

    return (
        prio_configs[list(prio_configs)[0]]["frame_size"],
        prio_configs[list(prio_configs)[0]]["levels"],
    )
...
def config_background(topology, traffic_items, level_id, packet_size):
    ...
    for prio_level, prio_config in prio_configs.items():
        ...
        for item in prio_config["traffic_items"]:
            ...
            if "destinations_groups" in prio_config:
                destination_groups = prio_config["destinations_groups"]
            else:
                destination_groups = [item]

            for devicegroup in topology.DeviceGroup.find():
                if "Upstream-" in devicegroup.Name:
                    upstream_src.append(devicegroup)
                elif "Downstream-" in devicegroup.Name:
                    downstream_src.append(devicegroup)
                else:
                    pass

            if "flow_groups" not in prio_config:
                continue

            packet_size = prio_config["frame_size"][0]

            for i in range(0, len(prio_config["flow_groups"])):
                d_groups = []
                merged_ussources = [
                    k
                    for k in upstream_src
                    if k.Name.replace("Upstream-", "") in destination_groups
                ]
                merged_dssources = [
                    k
                    for k in downstream_src
                    if k.Name.replace("Downstream-", "") in destination_groups
                ]
                ...
                if not endpoint:
                    endpoint = traffic_item.EndpointSet.add(
                        Name=item + prio_config["flow_groups"][i],
                        Sources=merged_ussources,
                    )

                start = 2
                if "start" in prio_config:
                    start = prio_config["start"]

                scalable_destinations = []
                for group in merged_dssources:
                    scalable_destinations.append(
                        {
                            "arg1": group.href + "/ethernet/1/ipv4/1",  # Very hacky
                            "arg2": 1,
                            "arg3": 1,
                            "arg4": start,  # Start
                            "arg5": prio_config["levels"][
                                level_id
                            ],  # Number of endpoints = arg5
                        }
                    )
                endpoint.ScalableDestinations = scalable_destinations
                ...
                bg_config_element.FrameRate.Rate = prio_config["data_rate"][i]
                bg_config_element.FrameSize.FixedSize = prio_config["frame_size"][i]
...
traffic_items = [
    item
    for sublist in traffic_items
    for item in (sublist if isinstance(sublist, list) else [sublist])
]  # flatten list
traffic_items = list(set(traffic_items))
traffic_items = sorted(traffic_items, key=lambda x: int(x[2:]))
